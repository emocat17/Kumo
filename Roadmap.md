...
- [x] **实时刷新**: 优化前端轮询机制，降低后端压力，保证数据实时性。
...
- [x] **资源占用监控**: 
    - [x] 后端使用 `psutil` 监控子进程的 CPU/内存占用，并记录历史峰值。
    - [x] 前端 `TaskHistoryModal` 展示任务执行的历史资源峰值 (CPU/Mem)。
...
### 5.1 基础集群能力
- [x] **节点标识 (Node Identity)**: 支持多节点运行时的身份识别 (`KUMO_NODE_ID`)，并在任务执行记录中标记运行节点。
...
### Phase 1: 稳固基础 (v0.5.0)
- **目标**: 修复现有 Bug，完善环境变量管理，优化 Docker 构建，提升前端交互体验。
- **重点**: 1.1, 1.2
- **说明**: 夯实地基，确保环境构建快、稳、好用，这是任务运行的前提。
- [x] **前端体验优化**:
    - [x] 修复操作审计表格列宽溢出问题。
    - [x] 优化系统设置页面 Tab 布局间距。
    - [x] 修复前端 TypeScript 类型定义错误。
