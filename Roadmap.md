...
- [x] **测试指标总览 (前端展示)**:
    - [x] 新增「测试指标」页面入口（Dashboard Tab）。
    - [x] 页面内一站式操作：选择项目、选择一个或多个任务、配置测试参数并一键执行。
    - [x] 指标卡片预览：数据源覆盖度、采集效率、处理速度、资源峰值。
    - [x] 图表与表格：按项目/任务维度展示趋势与对比。
    - [x] 动态指标看板：支持 1s/10s/1min 时间窗的实时曲线与缩略图预览。
    - [x] 全程指标图：单项目运行周期内的全量曲线与对比视图。
    - [x] 测试结果导出：JSON/CSV 报告，用于对外展示与留档。
- [ ] **采集效率测试**:
    - [x] 吞吐量指标：单位时间新增文件数与数据量统计。
    - [x] 延迟指标：任务启动、采集完成、输出落盘的时间分段统计。
    - [x] 资源占用：复用 max_cpu/max_memory 记录高负载稳定性。
    - [ ] 并发基准：批量并行任务模板与结果汇总视图。
- [ ] **数据处理速度测试**:
    - [ ] 处理阶段埋点：文档解析、索引构建、API 查询耗时上报。
    - [ ] 处理性能看板：阶段耗时对比与瓶颈定位。
    - [ ] 处理 SLA 规则：阈值配置与超限提示。
- [ ] **测试流程向导**:
    - [ ] 引导步骤：项目上传、环境构建、任务配置、日志监控、输出核验。
    - [ ] 一键生成测试任务：按数据源模板生成任务与触发器配置。

### Phase 6: 数据一致性与清理管理

- [x] **Settings 数据管理入口**:
    - [x] 新增 Settings Tab：集中预览 Python 版本、Python 环境、项目、任务四类数据。
    - [x] 展示数据库路径与存在状态，明确数据来源。
    - [x] 识别路径缺失状态，提示数据库记录与文件系统不一致。
- [x] **手动清理能力**:
    - [x] 允许按记录删除：版本、环境、项目、任务。
    - [x] 记录删除后刷新数据视图，避免残留显示。
    - [x] 删除按钮禁用与删除中提示，防止重复点击。
- [x] **一键清空能力**:
    - [x] 清空数据库全部表数据。
    - [x] 清理项目目录、环境目录、任务日志与安装日志。
    - [x] 停止调度任务，避免清空过程写入新数据。
- [x] **风险控制与提示**:
    - [x] 一键清空前强制确认提示。
    - [x] 明确仅清理系统管理目录，避免误删外部环境。
