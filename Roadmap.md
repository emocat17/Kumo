# Roadmap.md - Kumo 爬虫任务管理平台开发规划

## 核心目标
打造一个功能完善、易于管理的**单机/轻量级分布式**爬虫任务管理平台，具备以下核心特性：
- **可视化任务管理与监控** (已实现基础版本，需增强统计分析)
- **灵活的 Python 环境隔离** (已通过 Conda/Venv 实现)
- **高效的资源利用率** (需优化容器资源限制)
- **健壮的任务调度机制** (需补充重试、超时与告警)

---

## 1. 基础架构增强 (Infrastructure)
### 1.1 环境管理模块
- [ ] **自定义 pip 镜像源配置**: 支持用户在前端配置 PyPI 镜像源，加速依赖安装。
- [ ] **环境变量管理系统**:
    - [ ] 全局环境变量配置界面 (API Keys, DB URLs)。
    - [ ] 脚本级环境变量继承机制 (注入到 Task `subprocess` 中)。
    - [ ] 敏感信息 (`SECRET_KEY`) 加密存储方案。

### 1.2 容器化与部署优化
- [ ] **精简 Docker 镜像**: 优化 `backend` 和 `front` 的 Dockerfile，减少构建体积。
- [ ] **多阶段构建**: 分离构建环境与运行环境。
- [ ] **资源限制**: 在 `docker-compose.yml` 中添加 CPU/Memory 限制配置示例。

---

## 2. 监控与可视化 (Monitoring & Visualization)
### 2.1 仪表盘增强 (Dashboard)
- [ ] **任务执行概览面板**:
    - [ ] 统计总任务数、运行中、成功/失败次数。
    - [ ] 任务成功率环形图。
- [ ] **任务执行趋势分析**:
    - [ ] 过去 24 小时/7 天的任务执行量折线图。
    - [ ] 失败原因 Top 5 统计。

### 2.2 交互式图表
- [ ] **多维度筛选**: 基于项目、时间范围筛选日志和统计数据。
- [ ] **实时刷新**: 优化前端轮询机制，降低后端压力，保证数据实时性。

---

## 3. 任务管理功能 (Task Management)
### 3.1 调度系统升级 (Core)
*目前 Task 模型仅支持基础触发器，需扩展字段支持高级调度。*
- [ ] **失败自动重试**:
    - [ ] 数据库新增 `retry_count`, `retry_delay` 字段。
    - [ ] `task_manager` 实现重试逻辑。
- [ ] **任务超时控制**: 新增 `timeout` 字段，强制终止超时任务。
- [ ] **任务优先级**: 新增 `priority` 字段，用于调度队列排序。

### 3.2 执行监控
- [ ] **实时日志优化**: 解决长日志加载性能问题 (目前基于文件读取)。
- [ ] **资源占用监控**: 尝试通过 `psutil` 监控子进程的 CPU/内存占用。
- [ ] **异常告警**: 任务失败时通过 Webhook/Email 发送通知。

---

## 4. 系统可靠性 (System Reliability)
### 4.1 可靠性与审计
- [ ] **操作审计**: 记录关键操作 (删除环境、修改任务) 到审计日志表。
- [ ] **数据库自动备份**: 定时备份 `TaskManage.db`。

---

## 5. 分布式扩展 (Future)
*当前阶段专注于单机性能与体验，分布式作为长期目标。*
### 5.1 基础集群能力
- [ ] **节点标识**: 完善 `Task.node_id` 逻辑，允许 Worker 节点注册。
- [ ] **任务分发**: 简单的 HTTP 任务分发机制。

---

## 6. 生态集成 (Ecosystem)
### 6.1 框架模板支持
- [ ] **Scrapy 集成**: 自动识别 `scrapy.cfg`，提供 `scrapy crawl` 命令快捷生成。
- [ ] **Selenium/DrissionPage**: 提供包含 Headless 浏览器环境的 Docker 镜像变体。

---

## 实施策略与里程碑

### Phase 1: 稳固基础 (v0.5.0)
- **目标**: 修复现有 Bug，完善环境变量管理，优化 Docker 构建。
- **重点**: 1.1, 1.2
- **说明**: 夯实地基，确保环境构建快、稳、好用，这是任务运行的前提。

### Phase 2: 调度核心升级 (v0.8.0)
- **目标**: 完善任务调度逻辑，引入重试、超时和告警，确保任务执行的健壮性。
- **重点**: 3.1, 3.2
- **说明**: 增强核心业务能力，确保任务"能跑"且"跑得稳"，并产生详细的执行数据。

### Phase 3: 可视化与监控 (v1.0.0)
- **目标**: 基于调度数据构建仪表盘，提供全方位的系统洞察。
- **重点**: 2.1, 2.2, 4.1
- **说明**: 利用 Phase 2 产生的数据进行可视化展示，并完善审计与备份，达到正式版标准。

### Phase 4: 生态与扩展 (v1.5.0+)
- **目标**: 支持更多 Python 爬虫框架模板，探索分布式。
- **重点**: 6.1, 5.1

---

## 质量保证
- **单元测试**: 覆盖核心 `task_manager` 调度逻辑。
- **E2E 测试**: 确保前端操作与后端状态同步。
- **代码规范**: 严格遵循 PEP8 (Black/Flake8) 和 ESLint/Prettier。
