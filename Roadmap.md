...
- [ ] **测试指标总览 (前端展示)**:
    - [ ] 新增「测试指标」页面入口（Dashboard Tab）。
    - [ ] 页面内一站式操作：选择项目、选择一个或多个任务、配置测试参数并一键执行。
    - [ ] 指标卡片预览：数据源覆盖度、采集效率、处理速度、资源峰值。
    - [ ] 图表与表格：按项目/任务维度展示趋势与对比。
    - [ ] 动态指标看板：支持 1s/10s/1min 时间窗的实时曲线与缩略图预览。
    - [ ] 全程指标图：单项目运行周期内的全量曲线与对比视图。
    - [ ] 测试结果导出：JSON/CSV 报告，用于对外展示与留档。
- [ ] **采集效率测试**:
    - [ ] 吞吐量指标：单位时间新增文件数与数据量统计。
    - [ ] 延迟指标：任务启动、采集完成、输出落盘的时间分段统计。
    - [ ] 资源占用：复用 max_cpu/max_memory 记录高负载稳定性。
